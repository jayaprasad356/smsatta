...
$get_today_deposit = fetch(query("select sum(amount) as total from transactions where `created_at`>$sdate AND `created_at`<$edate AND remark like '%Points Added By Admin%'"));
$tdoay_deposit = $get_today_deposit['total']+0;

$get_today_withdraw = fetch(query("select sum(amount) as total from transactions where `created_at`>$sdate AND `created_at`<$edate AND remark like '%Points Withdraw By Admin%'"));
            ...
            <div class="card">
              ...
              <div class="card-body">
                 ...
                 <?php
                    ...
                    $select = mysqli_query($con, "SELECT * FROM `transactions` WHERE `created_at`>$sdate AND `created_at`<$edate AND ( remark like '%Points Added By Admin%' OR remark like '%Points Withdraw By Admin%' )"); 
...
$('#go').click(function(){
    ...
    if(date != ''){
        $.ajax({    //create an ajax request to 
            ...
            data:{date:date},  //expect html to be returned                
